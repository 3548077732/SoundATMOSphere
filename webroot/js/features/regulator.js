...
export const initRegulator = () => {
	...
	// Timbre slider for headphones
	const hTimbre = getDomElement('htimbre');
	if (hTimbre) {
		hTimbre.addEventListener('input', () => {
			const valueDisplay = getDomElement('htimbre-value');
			if (valueDisplay) {
				valueDisplay.textContent = convertToLanguageNumerals(hTimbre.value, lang);
			}
			updateOutput();
		});
	}

	// Timbre slider for speakers
	const sTimbre = getDomElement('stimbre');
	if (sTimbre) {
		sTimbre.addEventListener('input', () => {
			const valueDisplay = getDomElement('stimbre-value');
			if (valueDisplay) {
				valueDisplay.textContent = convertToLanguageNumerals(sTimbre.value, lang);
			}
			updateOutput();
		});
	}
